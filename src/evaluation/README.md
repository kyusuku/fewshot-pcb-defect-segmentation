...
- `metrics.py`: pure NumPy image AUROC, pixel AUROC, AUPRO, best threshold
  F1/IoU, and score-row helpers used by `scripts/evaluate_heatmaps.py`.
...
- `stage4.py`: combine heatmap `metrics.json` and mask `mask_metrics.json`
  files into the baseline-comparison table.
